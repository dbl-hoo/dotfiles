...
if [ -z "$monitor_id" ]
then
	idx_selected=$(printf '%b' "${windows_separators_formatted[@]}" |  rofi -dmenu -i -p "$command_" -a "$index_workspace_active" -format i -selected-row "$index_window_last_active" -config ~/.config/rofi/launchers/type-4/style-5.rasi -no-custom -s -width 80 -lines 30 -markup-rows -show-icons )
else
